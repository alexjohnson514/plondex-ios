    ...
    [NSURLConnection sendAsynchronousRequest:urlRequest queue:[NSOperationQueue currentQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError) {
            ...
            if ([jsonObject[KEY_SUCCESS] intValue] == 1) {
                ...
                if([jsonObject[KEY_DATA][KEY_USER_GROUP] isEqualToString: USERTYPE_USER])
                    [parentController.topViewController performSegueWithIdentifier:@"loginUserSegue" sender:self.presentingViewController];
                else if([jsonObject[KEY_DATA][KEY_USER_GROUP] isEqualToString: USERTYPE_VENDOR])
